...
impl BoardBevy {
    ...
    pub fn undo_move(
        ...
        // self.board.positions.castling_rights = castling_rights;

        // // Set the en_passant marker
        // if let Some(en_passant_tile) = en_passant_tile {
        //     self.board.positions.en_passant_tile = 1 << en_passant_tile.to_index();
        // }
        ...
        println!("Undone Move");
        let move_type = self.board.undo_move();

        match move_type {
            Some(PieceMoveType::EnPassant | PieceMoveType::Normal) => {
                // Create new entities for any captured pieces
                if let Some(captured_piece) = captured_piece {
                    // Set the captured piece tile, depending on if this capture was an en passant capture or not
                    let captured_piece_tile = if piece_move.move_type == PieceMoveType::EnPassant {
            ...
            Some(PieceMoveType::Promotion(_)) => {
                // Get the piece's player as an index
                let player_index = self
                    .board
                    .positions
